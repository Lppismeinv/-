...
def word_seg_eval(trainfile):
    ...
    # 保存分词结果
    with open('./src/data/人民日报分词结果.txt', 'w', encoding='utf8') as f:
...
if __name__ == "__main__":
    # 评估分词模型
    token_res = word_seg_eval('./src/data/人民日报语料（UTF8）.txt')

    # 评估词性标注
    # 在最大概率分词集合上标注词性
    trainfile = './src/data/人民日报词性标注版.txt'
    testfile = './src/data/人民日报分词结果.txt'
